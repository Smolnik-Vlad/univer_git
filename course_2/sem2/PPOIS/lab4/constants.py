...
class Constants(Sets_and_operations):
    ...
    def __get_saved_sets_str(cls, set_of_consts: list) -> dict:
        ...
        selected_saved_sets_str = {i: j.output_a_set(first_enter=True) for i, j in
                                   selectes_saved_sets_class.items()}  # вот здесь изменил вывод множества
    ...
    def read_line(self, line: str):
        ...
        if line.find('=') == -1:
            ...
            res = self.my_set.output_a_set(first_enter=True)
            return res
        else:
